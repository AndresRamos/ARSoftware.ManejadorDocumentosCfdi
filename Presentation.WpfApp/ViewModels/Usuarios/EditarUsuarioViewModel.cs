...
public sealed class EditarUsuarioViewModel : Screen
{
    ...
    public BindableCollection<EmpresaPerfilDto> EmpresasPermitidas { get; } = new();
    ...
    public async Task AgregarEmpresaPermitidaAsync()
    {
        try
        {
            IEnumerable<EmpresaPerfilDto> empresaPermitidas = await _mediator.Send(new BuscarEmpresasPermitidasPorUsuarioQuery(UsuarioId));
